...
error_exit:
    ...
    const bool was_suppressed = os_suppress_error_messages(true);

    if(os_file_delete(filename) < 0)
        msg_error(errno, LOG_ERR, "Failed to delete incomplete file");

    os_suppress_error_messages(was_suppressed);
